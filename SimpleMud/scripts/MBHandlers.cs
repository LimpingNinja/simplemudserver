...
// M - String, F string, p int, p row, p col, int abort0ls
    ...
    {
        public static void _on_error(IntPtr s, MBScript.mb_error_e e, string m, string f, int p, ushort row, ushort col, int abort_code) 
        {
            ...
            string type = abort_code == MBScript.MB_FUNC_WARNING ? "Warning" : "Error";
            if(e == MBScript.mb_error_e.SE_NO_ERR)
                return;

            if(f is not null) {
                if(e == MBScript.mb_error_e.SE_RN_REACHED_TO_WRONG_FUNCTION) {
                    Console.WriteLine(
                        $"{type}:\n    Ln {row}, Col {col} in Func: {f}\n    Code {e}, Abort Code {abort_code}\n    Message: {m}.\n");
                } else {
                    var x = e == MBScript.mb_error_e.SE_EA_EXTENDED_ABORT ? abort_code - MBScript.MB_EXTENDED_ABORT : abort_code;
                    Console.WriteLine(
        ...
        public static int _on_output(IntPtr s, string fmt, ArgIterator arglist) {
            nint pnt = default;
            MBScript.mb_get_userdata(s, ref pnt);
